...
function writeWrapperScript(repoPath: string): string {
  ...
  const safeGbrainBinDir = dirname(gbrainPath).replace(/'/g, "'\\''");
...
# Put the gbrain CLI's own dir on PATH so autopilot's subprocess spawns resolve
# it under launchd/systemd/cron, where rc files often don't export it (they
# early-return for non-interactive shells, or guard the PATH line behind
# interactivity). Derived from the install-time CLI path; placed after rc so rc
# can't clobber it.
export PATH='${safeGbrainBinDir}':"\$PATH"
